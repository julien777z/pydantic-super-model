...
echo '{"decision":"block","reason":"Before pushing the initial commit for this session, call the Skill tool with skill=\"code-simplify\" (the project skill, NOT the built-in \"simplify\" skill). Actually walk the skill against the code you modified this session: check each touched file against your rules and the skill defaults, and apply fixes — do not rubber-stamp with \"no changes needed\". Stage any simplifications into the same commit so the original code and the simplifications land in one commit and one push. Do not push a separate follow-up commit, and do not amend after pushing. If you modified no code at all, skip the skill and conclude."}'
