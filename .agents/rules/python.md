...
def record_event(event: Event, client_ip: str | None, user_agent: str | None) -> None:
    ...


# Good: one typed object describing the shared concept
def record_event(event: Event, audit_context: AuditContext) -> None:
    ...
```
...
- For cross-request or cross-process caching, use Redis (for example the `cache_backend()` helper backed by `fastapi-redis-sdk`'s `CacheBackend`) or another distributed caching library, and always set an explicit TTL.
...
- Prefer a test over a comment that explains a guard, ordering, or invariant. If a comment exists to justify why a branch, early return, or operation order is required (for example "we check X before Y so Z can't happen"), encode that guarantee in a test that fails when the behavior regresses, and delete the comment. The test is the durable, executable specification; the comment is not.
